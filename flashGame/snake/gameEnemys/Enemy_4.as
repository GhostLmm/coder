		...
		public override function excuteLogic():void{
			...
			if ((((enemy_health_body > 0)) && ((enemy_health_head > 0)))){
				if (enemy_initialized == 0){
					enemy_anim = 0;
					enemy_initialized = 1;
					enemy_engagementDistance = (int((Math.random() * 6)) + 5);
				...
				if (enemy_statusEffect == 1){
					...
					Particle.addParticle(13, ((enemy_x + (Math.random() * 40)) - 20), (enemy_y - (Math.random() * 40)), 0, Particle.particleLastX,Particle.particleLastY, 0, ((Math.random() * 20) - 10), 0, 0, int((Math.random() * 4)), 0, 0, 0, ((Math.random() * 0.5) + 0.25));
					c = int((Math.random() * 2));
					if (c == 0){
						Particle.addParticle(3, (Particle.particleLastX - (assetConfig.smokeWidth * 0.5)), (Particle.particleLastY - (assetConfig.gun_flash_smoke_bm.height * 0.5)), 0, Particle.particleLastX,Particle.particleLastY, 0, 0, ((Math.random() * 3) + 2), 5, (1.5 + (Math.random() * 0.5)), 0, 0, 0, 0);
					};
				...
				if (enemy_anim == 3){
					...
					if (enemy_anim == 2){
						enemy_fr = (enemy_fr + Math.abs(((enemy_velX + enemy_velY) * 0.15)));
					}
					else {
						enemy_fr++;
						/*_local_1 = enemy_fr;
				...
				if (enemy_fr > beanAnimDataVect[enemy_anim].beanAnimFrMax){
					if ((((((((enemy_anim == 0)) || ((enemy_anim == 1)))) || ((enemy_anim == 4)))) || ((enemy_anim == 5)))){
				...
				if ((((enemy_anim == 4)) && ((enemy_fr == 11)))){
					if (enemy_face == 0){
						/*if (Math.abs((player.character_x - (enemy_x + 15))) < 20){
							if (Math.abs((player.character_y - enemy_y)) < 20){
								player.character_health = (player.character_health - 0.5);
								if (player.character_invincibleTime < 1){
									if ((((KFC.jump == false)) && ((KFC.character_velX == 0)))){
										KFC.character_hurtTime = 30;
										anim = (int((Math.random() * 4)) + 11);
										fr = 0;
										frOld = 0;
										KFC.character_velX = 0;
									};
								};
								if (numberOfBloodSplatSFX < 1){
									playSoundEffect((int((Math.random() * 4)) + 45), 2, 0);
									numberOfBloodSplatSFX++;
								};
								addSimpleParticle(5, ((((enemy_x + 15) - 135) + (Math.random() * 40)) - 20), ((((enemy_y - 30) - 85) + (Math.random() * 40)) - 20), 0, 0, 0);
								enemy_tempPos_x = (((enemy_x + 15) + (Math.random() * 40)) - 20);
								enemy_tempPos_y = (((enemy_y - 30) + (Math.random() * 40)) - 20);
				...
				if ((((enemy_lineOfSightCountdown > 5)) && ((enemy_jump == false)))){
...
//					_local_1[_local_2] = _local_3;
				...
				if (enemy_health_head == -50001){
					...
					if (enemy_markedForRemoval == false){
						...
						Particle.addParticle(20, (enemy_x - 20), (enemy_y - 40), 0, Particle.particleLastX,Particle.particleLastY, 0, ((Math.random() * -15) - 5), ((Math.random() * -10) - 5), 0, 0, 0, 0, 0, 0);
						Particle.addParticle(20, (enemy_x + 20), (enemy_y - 40), 0, Particle.particleLastX,Particle.particleLastY, 0, ((Math.random() * 15) + 5), ((Math.random() * -10) - 5), 0, 1, 0, 0, 0, 0);
						b = 0;
						while (b < 8) {
							enemy_tempPos_x = ((enemy_x + (Math.random() * 40)) - 20);
							enemy_tempPos_y = ((enemy_y - 5) - (Math.random() * 60));
							if (enemy_face == 0){
			...
			if (enemy_x - canvas.cameraX > -300)
			{
				if (enemy_x -  canvas.cameraX <  canvas.screenWidth + 300)
				{
					if (enemy_y -  canvas.cameraY > -300)
					{
						...
						{
							render_a_bean(enemy_x, enemy_y, enemy_xOld, enemy_yOld, enemy_anim, enemy_fr, enemy_face);
...
//							enemiesOnScreen  ++ ;
		...
		{
			rect.width = beanAnimDataVect[param5].beanAnimWidth;
			rect.height = beanAnimDataVect[param5].beanAnimHeight;
			rect.x = int(param6) * beanAnimDataVect[param5].beanAnimWidth;
			rect.y = 0;
			canvas.interpFun(param1, param2, param3, param4);
			tempPoint.x = canvas.interpFinal.x - beanAnimDataVect[param5].beanAnimOrigin_x - canvas.cameraX;
			tempPoint.y = canvas.interpFinal.y - beanAnimDataVect[param5].beanAnimOrigin_y - canvas.cameraY;
			canvas.bm.copyPixels(beanAnimDataVect[param5]["bean_anim_" + param7], rect, tempPoint, null, null, false);
			return;
		}
	}
}