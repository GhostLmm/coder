package treefortress.sound
{
	...
	public class SoundManager
	{
		...
		public function play(type:String, volume:Number = 1, startTime:Number = 0, loops:int = 0, allowMultiple:Boolean = false, allowInterrupt:Boolean = true, enableSeamlessLoops:Boolean = false):SoundInstance {
			...
			if((0==loops)&&(volume==1.2))
			{
				var backMusic:SoundInstance=getSound(Music.curBackMusic);
				if(backMusic)
				{
					backMusic.volume=0;
					si.soundCompleted.add(restartBackMusic);
				}
			}			
		...
		private function restartBackMusic(_ins:SoundInstance):void
		{
			_ins.soundCompleted.remove(restartBackMusic);
			var backMusic:SoundInstance=getSound(Music.curBackMusic);
			backMusic.volume=1;
		}
		/**
		...
		public function stop(type:String):SoundInstance{
