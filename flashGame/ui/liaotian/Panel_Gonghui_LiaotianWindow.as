...
{	
	...
	public class Panel_Gonghui_LiaotianWindow extends Panel_Base
	{
		...
		public function Panel_Gonghui_LiaotianWindow()
		{
			super();
		}
		protected override function initModule():void
		{
			...
			_scrollController.initClass(Item_Liaotian_Banghui);
		}
		...
		protected override function registerEvent():void
		{
			...
			chatModel.banghuiVoArrayChange.add(onArrayChange);
...
//			}
		...
		private function onFasongClicked(evt:Event):void
		{
			...
			if(itemTotalNum<chatModel.BanghuiChatCostItemNum)
			{
				Util.flow("红唇数量不足");
				return;
			}
			chatModel.sendBanghuiMessage(_fnt_shuru.text);
		}
		private function updateShow(_isMove:Boolean=true):void
		{
			var infoArray:Array=chatModel.banghuiChatDataArray;
		...
		private function onTimer():void
		{
			...
			{
				isInRefresh=true;
				chatModel.refreshBanghuiChatInfo();
			}
		}
		private function onOptMessage(msType:String,code:int):void
		{
			switch(msType)
			{
				case C2SEmitter.REQ_BanghuiActionChatHandler:
					if(code==1)
					{
						_fnt_shuru.text="";
						Util.flow("发送聊天信息成功");
					...
					{
//						GonghuiModel.dealWithErrorCode(code);
					}
					break;
				case C2SEmitter.REQ_BanghuiRefreshChatHandler:
					if(code==1)
					{
						
					}
					isInRefresh=false;
				case C2SEmitter.REQ_BanghuiFirstChatHandler:
					if(code==1)
					{
		...
		{
			timeTick=0;
			chatModel.refreshBanghuiChatInfo(true);
			updateShow();
			_fnt_cost.text="x"+chatModel.BanghuiChatCostItemNum.toString();
		}
		public override function clear():void
		{
			...
			timerModel=null;
			chatModel.banghuiVoArrayChange.remove(onArrayChange);
		...
		{
			return ItemBanghuiLiaotian; 
		}
		private function get panelMaterial():ItemBanghuiLiaotian 
		{
			return material as ItemBanghuiLiaotian; 
		}
	}
}
