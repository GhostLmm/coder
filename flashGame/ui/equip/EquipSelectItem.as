package ui.equip
{
	...
	public class EquipSelectItem extends Base_Item implements IGridItemMax
	{
		private var _pos_item:Position;
		private var _mc_dagou:MovieClip;
		public function EquipSelectItem($mc:MovieClip=null)
		{
			super($mc);
		}
		...
		private function onTap(evt:Event):void{
			var mdiRonglian:MDI_EquipRonglian=Context.getInjectInstance(MDI_EquipRonglian);
			if(mdiRonglian){
				mdiRonglian.onScrSelected(index);
				return ;
			}
			var mdiTunshi:MDI_ShenzhuangTunshi=Context.getInjectInstance(MDI_ShenzhuangTunshi);
			if(mdiTunshi){
				mdiTunshi.onScrSelected(index);
				return ;
			}
			var mdiChuancheng:MDI_Chuancheng=Context.getInjectInstance(MDI_Chuancheng);
			if(mdiChuancheng){
				mdiChuancheng.onScrSelected(index);
				return ;
			}
		}
